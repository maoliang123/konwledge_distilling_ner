...
class BERT_LISM_CRF_student(nn.Module):

    def __init__(self, args,tagset_size,pretrain_emb):
        super(BERT_LISM_CRF_student, self).__init__()
        self.args=args
        if args.use_bert==True:
            ...
            self.encoder.weight.data.copy_(torch.from_numpy(pretrain_emb))
            self.lstm = nn.GRU(args.embedding, args.rnn_dim, num_layers=args.rnn_layer, batch_first=True,
                               bidirectional=True)
    ...
    def tag_outputs(self, input_ids, input_mask, targets,sen_len,outputs_teacher,mask_teacher):
        seq_length = input_ids.size(1)
        max=sen_len.max().item()
        ...
        # indices_teacher = torch.LongTensor([1, max])
        # indices_student = torch.LongTensor([0, max])
        if torch.is_tensor(outputs_teacher):
            l_t=outputs_teacher * mask_teacher.unsqueeze(-1).expand_as(outputs_teacher)
            l_s=output * input_mask.unsqueeze(-1).expand_as(output)
        else:
            l_s=0
            l_t=0
        return total_loss / batch_size, tag_seq,l_s,l_t,max
    ...
    def batchify(self, batch_list,teacher_h=0):
        batch_size = len(batch_list)
        sent_idx = [ele[0] for ele in batch_list]
        sent_ids = [ele[1] for ele in batch_list]
        targets = [ele[2] for ele in batch_list]
        if teacher_h!=0:
            outputs_teacher = [ele[3] for ele in batch_list]
            mask_teacher = [ele[4] for ele in batch_list]
        ...
        return input_ids, attention_mask, targets, info,outputs_teacher,mask_teacher
